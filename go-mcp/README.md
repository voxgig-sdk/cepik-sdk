...
[MCP](https://modelcontextprotocol.io) server exposing the Cepik SDK as
two agent tools — `cepik_list` and `cepik_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/cepik-mcp)
make build

# 2. Provide credentials via the environment
export CEPIK_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user cepik \
  -- /absolute/path/to/cepik-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./cepik-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// cepik_list: first page of records
{ "entity": "driving_license" }
{ "entity": "driving_license", "query": { } }

// cepik_load: one record by id
{ "entity": "statistic", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/cepik-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user cepik \
     -- "$PWD"/dist/*/cepik-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `cepik_list` and `cepik_load` tools now appear
   in new sessions. Ask the agent to *"list driving_license using cepik"*
   and it calls `cepik_list` with `{"entity":"driving_license"}`.
...
```sh
export CEPIK_APIKEY=sk_live_xxx            # API key
export CEPIK_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `cepik_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `cepik_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CEPIK_APIKEY` | API key sent with every request. |
| `CEPIK_BASE` | Optional override of the API base URL. |
...
```sh
./cepik-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"cepik_load","arguments":{"entity":"statistic","query":{"id":1}}}}'
```
