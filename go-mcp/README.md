...
MCP server exposing the Cepik SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `cepik_list` | `entity`, optional `query` map | First page of records as JSON |
| `cepik_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `cepik_list` and
`cepik_load` tools become available in new sessions.
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"cepik_load","arguments":{"entity":"driving_license","query":{"id":1}}}}'
```
