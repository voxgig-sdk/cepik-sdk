...
type Args struct {
	Entity string         `json:"entity" jsonschema:"driving_license | permission | statistic | vehicle"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.CepikSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "cepik_list",
		Description: "List records from Cepik. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "cepik_load",
		Description: "Load a single record from Cepik. " +
...
func runOp(client *sdk.CepikSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.CepikSDK, name string) (sdk.CepikEntity, error) {
	switch strings.ToLower(name) {
	case "driving_license":
		return client.DrivingLicense(nil), nil
	case "permission":
		return client.Permission(nil), nil
	case "statistic":
		return client.Statistic(nil), nil
	case "vehicle":
		return client.Vehicle(nil), nil
