...
class DrivingLicenseEntity
  ...
  def make
    opts = @_entopts.dup
    DrivingLicenseEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = CepikHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [DrivingLicense, Hash] the current DrivingLicense data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = CepikHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of DrivingLicense fields)
  def match_get
  ...
  #
  # @param reqmatch [DrivingLicenseListMatch, Hash, nil] match filter (any subset of
  #   DrivingLicense fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<DrivingLicense>, Array] the matching DrivingLicense items; raises CepikError on failure
