...
class PermissionEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Permission(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Permission(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CEPIK_TEST_PERMISSION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    permission_ref01_list_result = permission_ref01_ent.list(permission_ref01_match, nil)
    assert permission_ref01_list_result.is_a?(Array)
...
def permission_basic_setup(extra)
  ...
  entid_env_raw = ENV["CEPIK_TEST_PERMISSION_ENTID"]
  ...
  idmap_resolved = Helpers.to_map(
    env["CEPIK_TEST_PERMISSION_ENTID"])
