...
class VehicleDirectTest < Minitest::Test
  def test_direct_list_vehicle
    setup = vehicle_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-vehicle", setup[:live] ? "live" : "unit")
    if _should_skip
...
def vehicle_direct_setup(mockres)
