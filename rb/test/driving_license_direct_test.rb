...
class DrivingLicenseDirectTest < Minitest::Test
  def test_direct_list_driving_license
    setup = driving_license_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-driving_license", setup[:live] ? "live" : "unit")
    if _should_skip
...
def driving_license_direct_setup(mockres)
