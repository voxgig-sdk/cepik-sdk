...
func TestPermissionEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Permission(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Permission(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CEPIK_TEST_PERMISSION_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		permissionRef01Match := map[string]any{}

		permissionRef01ListResult, err := permissionRef01Ent.List(permissionRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, permissionRef01ListOk := permissionRef01ListResult.([]any)
		if !permissionRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", permissionRef01ListResult)
		}
...
func permissionBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"permission01", "permission02", "permission03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("CEPIK_TEST_PERMISSION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CEPIK_TEST_PERMISSION_ENTID"])
