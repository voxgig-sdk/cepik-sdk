...
import (
	...
	omni "github.com/voxgig-sdk/cepik-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CepikError); is {
				out := map[string]any{
					"name":    "CepikError",
...
func omniCtx(arg any, client *sdk.CepikSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CepikSDK); is {
			client = live
