...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("CEPIK_TEST_LIVE") == "TRUE" ||
		os.Getenv("CEPIK_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("CEPIK_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.CepikSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.CepikError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.CepikSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.CepikError{Msg: msg, Code: code}
}
