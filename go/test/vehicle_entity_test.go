...
func TestVehicleEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CEPIK_TEST_VEHICLE_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		vehicleRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.vehicle")))
...
func vehicleBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CEPIK_TEST_VEHICLE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CEPIK_TEST_VEHICLE_ENTID"])
