...
boru-driven command-line client **and** interactive REPL for the Cepik
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/cepik-cli)
make build
...
./cepik-cli list permission

# 5. Override the API base URL for a single call
CEPIK_BASE=https://api.example.com ./cepik-cli list driving_license

# 6. No arguments -> interactive REPL
./cepik-cli
cepik> list driving_license
cepik> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/cepik-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/cepik-cli list driving_license
   ```
...
Run with no arguments to open a REPL (prompt `cepik>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `driving_license`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/cepik-cli`. |
...
(`./cepik-cli <expr>`) and interactively (the REPL), and expressions compose the
