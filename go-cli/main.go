// Command Cepik-cli is a CLI + REPL for the Cepik SDK,
...
const entitiesHelp = "driving_license permission statistic vehicle"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CEPIK_APIKEY (api key), CEPIK_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CEPIK_APIKEY carries the API key and
	// CEPIK_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CEPIK_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CEPIK_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCepikSDK(opts)
