...
class CepikTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            CepikTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = CepikTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = CepikTestRunner.getenv("CEPIK_TEST_LIVE")
        override = CepikTestRunner.getenv("CEPIK_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = CepikTestRunner.getenv(key)
        ...
        explain = CepikTestRunner.getenv("CEPIK_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["CEPIK_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if CepikTestRunner._test_control is not None:
            return CepikTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                CepikTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            CepikTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = CepikTestRunner.load_test_control()
...
def load_env_local():
    CepikTestRunner.load_env_local()


def env_override(m):
    return CepikTestRunner.env_override(m)


def entity_data(v):
    return CepikTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return CepikTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return CepikTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return CepikTestRunner.load_test_control()


def live_client_options():
    return CepikTestRunner.live_client_options()


def live_delay_ms():
    return CepikTestRunner.live_delay_ms()
