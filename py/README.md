...
The Python SDK for the Cepik API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.DrivingLicense(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.Cepik(None).load(
    {"id": "test01"}, None
)
