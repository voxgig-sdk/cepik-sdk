...
{
    private static function utility(): CepikUtility
    {
        return new CepikUtility();
    }
    ...
    private static function ctx(array $over = []): CepikContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new CepikContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new CepikOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof CepikError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new CepikOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = CepikMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new CepikOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = CepikMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new CepikOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = CepikMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CepikSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = CepikMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new CepikResponse([]),
            'result' => new CepikResult([]),
        ]);
        [, $err] = CepikMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new CepikSpec(['step' => 's']),
            'response' => new CepikResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new CepikResult([]),
        ]);
        [, $err] = CepikMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new CepikSpec(['step' => 's']),
            'response' => new CepikResponse(self::resp(200, ['v' => 1])),
            'result' => new CepikResult([]),
        ]);
        [, $err] = CepikMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new CepikSpec(['step' => 's']),
            'response' => new CepikResponse(self::resp(200, ['v' => 2])),
            'result' => new CepikResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        CepikMakeResponse::call($ctx);
    ...
    {
        $preset = new CepikResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new CepikSpec([]),
            'response' => new CepikResponse([]),
            'result' => new CepikResult([]),
        ...
        [$got, $err] = CepikMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new CepikResult([])]);
        [, $err] = CepikMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new CepikSpec([]), 'result' => null]);
        [, $err] = CepikMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CepikSpec(['step' => 's']),
            'result' => new CepikResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new CepikOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CepikMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CepikSpec(['step' => 's']),
            'result' => new CepikResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new CepikOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CepikMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CepikResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new CepikSpec([]),
            'result' => new CepikResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = CepikMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $utility = self::utility();
        $boom = new CepikError('boom', 'boom');
        $utility->fetcher = function (CepikContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CepikSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = CepikMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (CepikContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = CepikMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(CepikResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (CepikContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CepikSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = CepikMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new CepikResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new CepikSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = CepikMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = CepikMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = CepikMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new CepikResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, CepikDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new CepikResult(['ok' => false])]);
        $this->expectException(CepikError::class);
        CepikDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new CepikResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', CepikMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new CepikBaseFeature();
        $b = new CepikBaseFeature();
        CepikFeatureAdd::call($ctx, $a);
        CepikFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): CepikBaseFeature
    {
        $f = new CepikBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    private static function auth_ctx(array $options, ?array $headers): CepikContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new CepikSpec(['headers' => $headers]),
        ]);
    }
    ...
    {
        $ctx = self::ctx([
            'response' => new CepikResponse(['headers' => null]),
            'result' => new CepikResult([]),
        ]);
        CepikResultHeaders::call($ctx);
