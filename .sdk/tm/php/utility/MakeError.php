...
// Cepik SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class CepikMakeError
{
    public static function call(?CepikContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new CepikContext([], null);
        }
        $op = $ctx->op ?? new CepikOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new CepikResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof CepikError) ? $err->msg : (string)$err;
        $msg = "CepikSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new CepikError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof CepikError) {
