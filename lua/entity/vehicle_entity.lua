...
local VehicleEntity = {}
VehicleEntity.__index = VehicleEntity


function VehicleEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, VehicleEntity)
  self._name = "vehicle"
  self._client = client
...
function VehicleEntity:get_name()
  return self._name
end


function VehicleEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return VehicleEntity.new(self._client, opts)
end
...
function VehicleEntity:mark_deleted()
  self._deleted = true
end


function VehicleEntity:deleted()
  return true == self._deleted
end


function VehicleEntity:data_set(args)
  if args ~= nil then
...
function VehicleEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function VehicleEntity:match_set(args)
  if args ~= nil then
...
function VehicleEntity:match_get()
...
function VehicleEntity:stream(action, args, callopts)
...
function VehicleEntity:_run_op(ctx, post_done)
  local utility = self._utility
