...
local PermissionEntity = {}
PermissionEntity.__index = PermissionEntity


function PermissionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PermissionEntity)
...
function PermissionEntity:get_name()
  return self._name
end


function PermissionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PermissionEntity.new(self._client, opts)
end
...
function PermissionEntity:mark_deleted()
  self._deleted = true
end


function PermissionEntity:deleted()
  return true == self._deleted
end


function PermissionEntity:data_set(args)
  if args ~= nil then
...
function PermissionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PermissionEntity:match_set(args)
  if args ~= nil then
...
function PermissionEntity:match_get()
...
function PermissionEntity:stream(action, args, callopts)
...
function PermissionEntity:_run_op(ctx, post_done)
  local utility = self._utility
