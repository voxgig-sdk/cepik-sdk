...
describe("PermissionEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Permission(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Permission(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set CEPIK_TEST_PERMISSION_ENTID JSON to run live")
      return
    end
    ...
    local permission_ref01_match = {}

    local permission_ref01_list_result, err = permission_ref01_ent:list(permission_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(permission_ref01_list_result)
...
function permission_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("CEPIK_TEST_PERMISSION_ENTID")
