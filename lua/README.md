...
The Lua SDK for the Cepik API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:DrivingLicense(nil):list(nil, nil)
...
local result, err = client:Cepik(nil):load(
  { id = "test01" }, nil
)
