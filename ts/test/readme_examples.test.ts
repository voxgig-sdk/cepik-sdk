...
//     (\`new CepikSDK(...)\` / \`CepikSDK.test(...)\` become
//     \`CepikSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"driving_license":{"test01":{"id":"test01"}},"permission":{"test01":{"id":"test01"}},"statistic":{"test01":{"id":"test01"}},"vehicle":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new CepikSDK(...)\` and any
// \`CepikSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CepikSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CepikSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
