...
class CepikSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = CepikHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = CepikHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!CepikFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, CepikFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return CepikUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = CepikHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = CepikHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = CepikHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new CepikError($op . "_allow",
                "CepikSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = CepikHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new CepikError("graphql_error",
                "CepikSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function DrivingLicense($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_driving_license === null) {
                $this->_driving_license = new DrivingLicenseEntity($this, null);
            }
            return $this->_driving_license;
        }
        return new DrivingLicenseEntity($this, $data);
    }
