...
//    only - a typed CepikContext there would make every ctx
...
final class CepikOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new CepikContext($clean, null);
    ...
    public static function err_from_map($m): ?CepikError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new CepikError(is_string($code) ? $code : '', $msg);
    }
