...
class PermissionEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Permission(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Permission(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CEPIK_TEST_PERMISSION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function permission_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["permission01", "permission02", "permission03"] as $k) {
    ...
    $entid_env_raw = getenv("CEPIK_TEST_PERMISSION_ENTID");
