...
class FtCtrl extends CepikControl
{
...
class FtClient
{
    ...
    private CepikUtility $utility;

    public function __construct(CepikUtility $utility, array $options)
    {
    ...
    public function get_utility(): CepikUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CepikContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CepikContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CepikConfig::make_feature($name);
    ...
    private static function buildUrl(CepikSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CepikError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CepikResponse($fetched);
        }
    ...
    private function fail(CepikContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CepikContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
