...
// Cepik SDK DrivingLicense entity
...
class DrivingLicenseEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new DrivingLicenseEntity($this->_client, $opts);
    }

    /**
     * @param DrivingLicense|array $args DrivingLicense data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = CepikHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return DrivingLicense|array The current DrivingLicense data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of DrivingLicense fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = CepikHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of DrivingLicense fields).
     */
    ...
    /**
     * List DrivingLicense items matching the given filter.
     *
     * @param DrivingLicenseListMatch|array|null $reqmatch Match filter (any subset
     *   of DrivingLicense fields) as an assoc-array; DrivingLicenseListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return DrivingLicense[]|array A list of DrivingLicense items as assoc-arrays at
     *   the SDK boundary; throws CepikError on failure (item-5 convention).
     */
